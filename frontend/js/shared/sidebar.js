(function () {
  ...
  let items = [['dashboard','Overview','dashboard.html'],['generate-vouchers','Generate vouchers','generate-vouchers.html'],['users','Users','users.html'],['sessions','Sessions','sessions.html'],['revenue','Revenue','revenue.html'],['analytics','Analytics','analytics.html'],['packages-admin','Packages','packages.html'],['resellers','Resellers','resellers.html']];
  if (localStorage.getItem('adminRole') === 'SCHOOL_ADMIN') items = [['resellers','Resellers','resellers.html']];
  ...
  document.getElementById('adminLogout').onclick=()=>{localStorage.removeItem('adminApiKey');localStorage.removeItem('adminUsername');localStorage.removeItem('adminRole');location.href='adminlogin.html';};
})();
